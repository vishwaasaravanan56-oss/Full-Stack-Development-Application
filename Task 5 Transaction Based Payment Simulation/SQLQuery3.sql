...
DECLARE @MerchantId INT = 3;      -- Paying user (Kumar) -- Merchant (AmazonStore) 
DECLARE @Amount DECIMAL(12,2) = 90000.00; 
BEGIN TRY 
BEGIN TRANSACTION; -- 1) Check user has enough balance 
...
END -- 2) Deduct from user 
UPDATE Accounts 
SET Balance = Balance - @Amount 
WHERE AccountId = @UserId AND AccountType='USER'; -- Safety check: ensure exactly 1 row updated 
IF @@ROWCOUNT <> 1 
BEGIN 
RAISERROR('User account not found / update failed.', 16, 1); 
END -- 3) Add to merchant 
UPDATE Accounts 
SET Balance = Balance + @Amount 
WHERE AccountId = @MerchantId AND AccountType='MERCHANT'; -- Safety check: ensure exactly 1 row updated 
IF @@ROWCOUNT <> 1 
BEGIN 
